            ...
            <div class="row">
                        ...
                        @forelse($experts as $expert)
                            <div class="col-md-6">
                                <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden text-center">
                                    <img src="{{ Storage::disk('public')->url($expert->photo) }}" class="blog-card-img"
                                        alt="{{ $expert->name }}"
                                        onerror="this.onerror=null; this.src='{{ asset('assets/images/user-placeholder.png') }}';">
                                    <div class="card-body p-4">
                                        <h5 class="fw-bold mb-1"><a href="{{ route('expert', $expert->slug) }}"
                                                class="text-decoration-none text-dark">{{ $expert->name }}</a></h5>
                                        <p class="text-primary small mb-3 fw-bold">{{ $expert->position }}</p>
                                        <p class="text-muted small">{{ Str::limit(strip_tags($expert->bio), 80) }}</p>
                                        <a href="{{ route('expert', $expert->slug) }}"
                                            class="btn btn-outline-primary btn-sm rounded-pill mt-2 px-4">View Profile
                                            &rarr;</a>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col-lg-4 mt-5 mt-lg-0 ps-lg-5">
                    ...
                    <div class="bg-white p-4 rounded-4 shadow-sm mb-4">
                        ...
                        <a href="{{ route('experts.index', ['search' => request('search')]) }}"
                            class="category-link {{ !request('position') ? 'text-primary fw-bold' : '' }}">All Positions</a>
                        @foreach($positions as $pos)
                            <a href="{{ route('experts.index', ['position' => $pos, 'search' => request('search')]) }}"
                                class="category-link {{ request('position') == $pos ? 'text-primary fw-bold' : '' }}">
                                {{ $pos }}
                            </a>
                        @endforeach
                    </div>
                    ...
                    <div class="bg-primary p-4 rounded-4 shadow-sm text-white text-center">
                        ...
                        <a href="{{ url('/register') }}" class="btn btn-light text-primary fw-bold w-100 mb-2">Apply as an
                            Expert</a>
                    </div>
                </div>
            </div>
